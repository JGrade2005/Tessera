...
public final class ShapePreset {
    ...
    public static final class Param {
        ...
        public Param(String label, double min, double max, double def, boolean integer) {
            this.label = label; this.min = min; this.max = max; this.def = def; this.integer = integer;
        }
    }
    ...
    private ShapePreset(String name, String pattern, boolean hollow,
                        Function<double[], String> builder, Param... params) {
        this.name = name;
    ...
    public String build(double[] p) { return builder.apply(p); }
    ...
    /**
     * A torus of major radius R, minor radius rr, tilted by rotDeg about the X
     * axis. The rotation is baked into the coordinates (y,z) so the resulting
     * expression is a valid, copy-pasteable //g command. dataPrefix optionally
     * prepends a "data=...;" coloring clause.
     */
    ...
    public static final ShapePreset[] PRESETS = new ShapePreset[] {
        ...
        new ShapePreset("Mountain Peak", "stone", false,
            p -> "y < " + f(p[0]) + " - 1.6*sqrt(x^2+z^2) + " + f(p[1]) + "*sin(x*7) + "
                 + f(p[1]) + "*cos(z*8) + " + f(p[1] * 0.6) + "*sin(x*16+z*14)",
            new Param("height", 0.3, 1.2, 0.8, false),
            new Param("roughness", 0.0, 0.4, 0.2, false)),

        new ShapePreset("Mountain Range", "stone", false,
            p -> "y < -0.1 + " + f(p[0]) + "*sin(x*" + f(p[1]) + ") + " + f(p[0] * 0.9)
                 + "*cos(z*" + f(p[1] * 0.8) + ") + " + f(p[0] * 0.5) + "*sin(x*6+z*5) + 0.12*cos(x*11)",
            new Param("height", 0.1, 0.7, 0.45, false),
            new Param("ridges", 1.5, 5.0, 2.5, false)),

        new ShapePreset("Rolling Hills", "grass_block", false,
            p -> "y < -0.35 + " + f(p[0]) + "*sin(x*3)*cos(z*3) + " + f(p[0] * 0.7)
                 + "*sin(x*5+1) + " + f(p[0] * 0.5) + "*cos(z*4)",
            new Param("height", 0.15, 0.6, 0.3, false)),

        new ShapePreset("Canyon", "sandstone", false,
            p -> "y < 0.3 && y < -0.6 + " + f(p[1]) + "*abs(x - " + f(p[0]) + "*sin(z*2.5))",
            new Param("winding", 0.0, 0.5, 0.35, false),
            new Param("steepness", 1.5, 4.0, 2.5, false)),

        new ShapePreset("Mesa / Butte", "red_sandstone", false,
        ...
        new ShapePreset("Custom", "stone", false,
            p -> "x^2+y^2+z^2 < 0.8^2"),
    };
}
