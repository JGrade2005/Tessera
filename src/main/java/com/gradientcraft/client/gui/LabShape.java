...
public final class LabShape {
    ...
    public static String build(double[] s, boolean[] c) {
        double size = s[0], e = s[1], sx = s[2], sy = s[3], sz = s[4], taper = s[5],
               tw = s[6], rf = s[7], ra = s[8], wf = s[9], wa = s[10], thick = s[11], rot = s[12];
        boolean twist = c[0], ripple = c[1], wobble = c[2], hollow = c[3], invert = c[4], rainbow = c[5];

        double th = Math.toRadians(rot), cR = Math.cos(th), sR = Math.sin(th);
        List<String> st = new ArrayList<>();

        if (rainbow) st.add("data=(32+15/2/pi*atan2(x,y))%16");

        // rotation about the X axis (tilt)
        st.add("yr=" + (rot != 0 ? "(y*(" + f(cR) + ")-z*(" + f(sR) + "))" : "y"));
        st.add("zr=" + (rot != 0 ? "(y*(" + f(sR) + ")+z*(" + f(cR) + "))" : "z"));

        // twist the cross-section with height
        if (twist) {
            st.add("xt=(x*cos(" + f(tw) + "*yr)-zr*sin(" + f(tw) + "*yr))");
            st.add("zt=(x*sin(" + f(tw) + "*yr)+zr*cos(" + f(tw) + "*yr))");
        } else {
            st.add("xt=x");
            st.add("zt=zr");
        }
