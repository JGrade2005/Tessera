...
/**
 * Evaluator for the subset of the WorldEdit //generate expression language the
 * preview needs: x/y/z inputs, assignments (including += -= *= /=), the special
 * "data" variable, arithmetic, comparisons, logical operators, the usual maths
 * functions, and ';'-separated statements with an optional return.
 *
 * A block is "placed" when the final value is greater than 0. An assignment to
 * "data" is reported so the preview can colourise (e.g. rainbow wool).
 *
 * Compile once, evaluate many times. Not thread-safe: one shared environment.
 */
public final class ExprEngine {
        ...
        public Result eval(double x, double y, double z) {
            ...
            for (Stmt s : statements) {
                if (s.kind == Stmt.RETURN) {
                    ...
                    if ("+=".equals(s.op)) rhs = cur + rhs;
                    else if ("-=".equals(s.op)) rhs = cur - rhs;
                    else if ("*=".equals(s.op)) rhs = cur * rhs;
                    else if ("/=".equals(s.op)) rhs = cur / rhs;
    ...
    private static final class Tok {
        ...
        Tok(int type, String text, double num) {
            this.type = type;
            this.text = text;
            this.num = num;
        }
    ...
    private static List<Tok> tokenize(String s) {
        ...
        while (i < n) {
            ...
            if (two.equals("<=") || two.equals(">=")
                ...
                || two.equals("*=")
                || two.equals("/=")) {
                out.add(new Tok(Tok.OP, two, 0));
                i += 2;
                continue;
            }
            if ("+-*/%^(),;<>=!".indexOf(c) >= 0) {
                out.add(new Tok(Tok.OP, String.valueOf(c), 0));
                i++;
                continue;
            }
    ...
    private static final class Parser {
        ...
        private boolean isOp(String s) {
            Tok k = peek();
            return k.type == Tok.OP && k.text.equals(s);
        }
        ...
        private void expect(String s) {
            if (!eat(s)) throw new ParseException("Expected '" + s + "'");
        }
        ...
        private Node parseCmp() {
            ...
            while (isOp("<") || isOp(">") || isOp("<=") || isOp(">=")) {
                ...
                Node b = parseAdd();
                final Node aa = a;
                if ("<".equals(o)) a = v -> aa.eval(v) < b.eval(v) ? 1.0 : 0.0;
                else if (">".equals(o)) a = v -> aa.eval(v) > b.eval(v) ? 1.0 : 0.0;
                else if ("<=".equals(o)) a = v -> aa.eval(v) <= b.eval(v) ? 1.0 : 0.0;
                else a = v -> aa.eval(v) >= b.eval(v) ? 1.0 : 0.0;
            }
            return a;
        }
        ...
        private Node parseMul() {
            ...
            while (isOp("*") || isOp("/") || isOp("%")) {
                ...
                Node b = parseUnary();
                final Node aa = a;
                if ("*".equals(o)) a = v -> aa.eval(v) * b.eval(v);
                else if ("/".equals(o)) a = v -> aa.eval(v) / b.eval(v);
                else a = v -> aa.eval(v) % b.eval(v);
            }
            return a;
        }
    ...
    private static double applyFunc(String name, List<Node> a, Map<String, Double> v) {
        double a0 = a.isEmpty() ? 0
            : a.get(0)
                .eval(v);
        double a1 = a.size() > 1 ? a.get(1)
            .eval(v) : 0;
        switch (name) {
            case "sin":
                return Math.sin(a0);
